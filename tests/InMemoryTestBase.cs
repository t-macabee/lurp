...
public abstract class InMemoryTestBase : IDisposable
{
    ...
    public sealed class Extraction
    {
        ...
        public List<EdgeRecord> EdgesOf(string kind, string provenance = Provenance.CompilerProved)
            => Result.Edges.Where(e => e.Kind == kind && e.Provenance == provenance).ToList();

        public string ResolveId(string fqn)
            => FqnToSymbolId.TryGetValue(fqn, out var id)
                ? id
                : throw new InvalidOperationException($"No declared symbol with FQN '{fqn}'.");

        public EdgeRecord SingleEdge(string kind, string sourceFqn, string targetFqn, string provenance = Provenance.CompilerProved)
        {
        ...
        string? snapshotId = null)
    {
        ...
        workspace.AddSolution(SolutionInfo.Create(
            solutionId, VersionStamp.Create(), filePath: Path.Combine(_tempDir, "Test.slnx")));
        ...
        var compilation = (CSharpCompilation)(await project.GetCompilationAsync() ?? throw new InvalidOperationException("Compilation failed to build."));

        var workspaceInfo = new WorkspaceInfo(workspace.CurrentSolution, _tempDir);

        var options = CompilationFactExtractor.CreateOptions(
            skipAdapters: runAdapters ? null : AllAdapterNames);
