...
/// <summary>
/// Direct unit tests for <see cref="CompilationFactExtractor.RunStage"/>, the
/// helper all six <see cref="CompilationFactExtractor.ExtractAll"/> stages
/// (SymbolDeclaration, StructuralEdge, MemberEdge, Polymorphism, Reflection,
/// Adapter) now share. Testing the helper directly proves the uniform
/// failure/incompleteness contract for every call site at once, including
/// the Polymorphism stage that was previously unguarded.
/// </summary>
public sealed class CompilationFactExtractorRunStageTests
{
    ...
    {
        // Regression coverage for the audit finding: Polymorphism used to run
        // unguarded in ExtractAll, so a thrown exception here escaped the
        // whole extraction pipeline. It now goes through RunStage like every
        // other stage.
        var ctx = NewContext();
