...
- **Incremental re-extraction is document-scoped.** `IncrementalIndexer` no longer re-extracts every document of every affected project. `ExtractReplacementFacts` receives the extraction scope (changed documents ∪ reverse-edge closure ∪ every document declaring a part of a touched type), converted to the absolute forward-slash form the extraction guards compare against, and `PrepareSnapshotData` deletes over exactly the same set — extraction and deletion narrow in lockstep, so no fact is deleted for a document that is not re-extracted. Adapters still walk the whole compilation (`AdapterExtractionContext.ScopeDocuments` is plumbed but unhonored); their re-emitted edges are absorbed by the `INSERT OR IGNORE` on `ux_edges_relation`.
...
- **Adapters honor the extraction scope.** Five of the six now skip out-of-scope work: `DependencyInjectionAdapter` and `SerializationAdapter` guard their `compilation.SyntaxTrees` loop with `IsInScope`, `TestAdapter` guards each declaring syntax reference, and `AspNetCoreAdapter` and `MediatRAdapter` guard by declaring type via `AdapterExtractionContext.IsSymbolInScope`. In every case the guarded unit is the one the emitted edge is anchored to, so extraction and deletion stay on the same set. `EfCoreAdapter` remains deliberately unscoped: its walk also produces annotations, and annotations have no path-scoped delete, so scoping it would strand copied-forward annotation rows. Scoping EF Core requires an annotation delete path first.
