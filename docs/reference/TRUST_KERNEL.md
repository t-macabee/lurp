...
|---|---|---|
| 2026-08-06 | Capsule audit mitigation Task 9 decided (Option B) + registration tier reaches type-level facts | Task 9 closed as **out of scope**: sibling similarity is an inferred ranking with no compiler oracle, no provenance, and no completeness claim a `--strategy=full` rebuild could verify, so it violates the invariant that every capsule field is a proved fact or a declared absence. Recorded as registry entry `shape_similarity` (registry now 6 entries) and in `CAPSULE_AUDIT_MITIGATION.md` §Task 9, including the recorded nuance that shared-collaborator overlap is derivable but still declined because the *interpretation* is not provable. Found while testing the decision against the in-repo fixture: `RegisteredImplementationsTierBuilder` and `ContextTierContext.HasUnmodeledRegistrations` consulted only the anchor's own symbol ids, but DI registration is a type-level fact — so a capsule anchored on a *member* of a registered type (e.g. `RentalNotificationOutboxPublisher.ExecuteAsync`) saw the `AddHostedService<T>` registration only as an uncertainty, never in the tier named after it. Both now expand the anchor to its declaring types via `GetDeclaringTypeIds`; the type-anchor direction was already covered by `EffectiveSymbolIds`. Also corrected Task 5's scope wording: the implemented constraint-attachment rule is entity reachability, not `IQueryable<T>` participation. Tests: new `Finding1_MemberAnchor_SurfacesTypeLevelRegistration`; verified scoreboard 8/8 and adjacent capsule filters 44/44; full-suite run pending user-side. |
