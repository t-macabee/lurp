# Trust Kernel: Change Log (archive)

**Status:** historical implementation records, preserved verbatim from earlier
revisions of `TRUST_KERNEL.md`. This file is an archive, not a task queue.
For the current state of the system, read `TRUST_KERNEL.md`; entries here are
linked from its "Change log" table. Dates run 2026-07-27 → 2026-08-04.
...
Prompted by an inspection of six temporary capsule-output artifacts. The
language-version work above had already *diagnosed* the underlying condition
for eNote: no TFM → `ResolvePackageAssets` fails → no metadata references: but
nothing **enforced** it. Lurp continued to index reference-less compilations,
mark the snapshot `complete`, and serve capsules from the resulting near-empty
graph.
...
### Open findings for follow-up (original text)
...
1. **Capsules resolve callers through `MayDispatchTo` (closed 2026-08-03; two
   ...
   in the temporary capsule-output artifacts shows 12
   ...
   on.
2. **Temporary capsule-output artifacts are stale and pre-fix: do not treat them as
...
local generated exports and one-off
...
## Order 12 findings: Phase 9/10 gap audit (original text)
...
## Architecture alignment and recorded deviations (original text)
...
(`3981bdce…`, the retained capsule-output artifact): two
