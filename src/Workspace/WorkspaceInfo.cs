...
public sealed class WorkspaceInfo
{
        ...
        BuildDocumentMap(Solution solution, string gitRoot)
    {
        ...
        foreach (var project in solution.Projects)
        {
            foreach (var document in project.Documents)
            {
                ...
                if (IsGeneratedDocument(document, bytes, relPath, encoding))
                {
    ...
    private static bool IsGeneratedDocument(Document document, byte[] bytes, string relPath, string encoding)
    {
        if (IsGeneratedPath(relPath))
            return true;

        if (IsGeneratedHeader(bytes, encoding))
            return true;
    ...
    private static bool IsGeneratedHeader(byte[] bytes, string encoding)
    {
        if (bytes.Length == 0)
            return false;

        var headerLength = Math.Min(512, bytes.Length);
        var headerText = ResolveEncoding(encoding).GetString(bytes, 0, headerLength);
