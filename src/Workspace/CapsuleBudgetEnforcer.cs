...
internal static class CapsuleBudgetEnforcer
{
    ...
    private static bool BoundAnchorSourceToFit(ContextCapsule capsule, int budget)
    {
        ...
        capsule.Anchor.Source = string.Concat(source.AsSpan(0, contentChars), keepsMarker ? SourceTruncationMarker : string.Empty);
        return true;
    }
    ...
    {
        return items.Sum(static item => item.Source?.Length ?? 0);
    }

    private static int SerializedChars<T>(T value)
    {
        return JsonSerializer.Serialize(value, SectionOptions).Length;
    }
    ...
    private static void RecordTruncation(ContextCapsule capsule, TruncationEntry entry)
    {
        ...
        var existing = capsule.OmittedTiers.FindIndex(record => string.Equals(record.Category, entry.Category, StringComparison.Ordinal));
        if (existing >= 0)
    ...
    private static CapsuleItem BoundItemSource(CapsuleItem item, int maxChars)
    {
        ...
        var bounded = string.Concat(source.AsSpan(0, maxChars), SourceTruncationMarker);
        return new CapsuleItem(
            ...
            item.DocumentPath != null
                ? new DeclarationLocation(item.DocumentPath, item.StartLine ?? 0, item.StartColumn ?? 0, item.EndLine ?? 0, item.EndColumn ?? 0, false)
                : null,
    ...
    private sealed class SectionTrimmer
    {
        ...
        public SectionTrimmer(ContextCapsule capsule, IReadOnlyList<string> tierPriority)
        {
            ...
            var sections = tierPriority
                .Select(name => (name, items: TierItems(capsule, name)))
                .Where(x => x.items != null)
                .Select(x => TrimmableSection.Clear(x.name, x.items!))
                .ToList();
            ...
            var surrounding = sections.FirstOrDefault(static section => section.Name == "surrounding_source");
            if (surrounding != null)
            {
    ...
    private sealed class TrimmableSection(string name, Queue<TrimStep> steps)
    {
        ...
        {
            return new TrimmableSection(name, new Queue<TrimStep>([new TrimStep("budget_exhausted", ClearStep(items))]));
        }

        public static TrimmableSection ClearDictionary(string name, Dictionary<string, string> items)
        {
            return new TrimmableSection(name, new Queue<TrimStep>([new TrimStep("budget_exhausted", ClearDictionaryStep(items))]));
        }

        public static TrimmableSection WithSteps(string name, params TrimStep[] steps)
        {
            return new TrimmableSection(name, new Queue<TrimStep>(steps));
        }
    }
