...
public class SqliteIndexStore : IIndexStore, IDisposable
{
    ...
    {
        EnsureOpen();
        return _lifecycle!.GetSnapshotGitRoot(snapshotId);
    }

    public string? GetSnapshotStatus(string snapshotId, string workspaceId)
    {
        EnsureOpen();
        return _lifecycle!.GetSnapshotStatus(snapshotId, workspaceId);
    }

    public List<string> GetSnapshotIds(string workspaceId)
    {
        EnsureOpen();
        return _lifecycle!.GetSnapshotIds(workspaceId);
    }
    ...
    {
        EnsureOpen();
        return _documents!.GetDocumentVersionIdsByPath(snapshotId);
    }

    public List<string> GetDocumentVersionIdsForDocuments(string snapshotId, IEnumerable<string> documentPaths)
    {
        EnsureOpen();
        return _documents!.GetDocumentVersionIdsForDocuments(snapshotId, documentPaths);
    }
    ...
    {
        EnsureOpen();
        return _symbols!.GetSymbolIdsInSnapshot(snapshotId);
    }

    public int CountSymbolsInSnapshot(string snapshotId)
    {
        EnsureOpen();
        return _symbols!.CountSymbolsInSnapshot(snapshotId);
    }

    public void DeleteIncompleteSnapshots()
    {
        EnsureOpen();
        _pruner!.DeleteIncompleteSnapshots();
    }

    public void PruneOldSnapshots(int keep = 3)
    {
        EnsureOpen();
        _pruner!.PruneOldSnapshots(keep);
    }
    ...
    {
        EnsureOpen();
        _bindingIncompletenessStore!.SaveBindingIncompleteness(snapshotId, records);
    }

    public List<BindingIncompletenessRecord> GetBindingIncompleteness(string snapshotId, string? projectName = null)
    {
        EnsureOpen();
        return _bindingIncompletenessStore!.GetBindingIncompleteness(snapshotId, projectName);
    }
    ...
    {
        EnsureOpen();
        _declMaintenance!.DeleteDeclarationsByDocumentVersionIds(documentVersionIds);
    }

    public List<string> GetSymbolIdsByDocumentVersionIds(string snapshotId, IEnumerable<string> documentVersionIds)
    {
        EnsureOpen();
        return _declMaintenance!.GetSymbolIdsByDocumentVersionIds(snapshotId, documentVersionIds);
    }

    public string? ResolveSymbolByLocation(string relativePath, int line, string snapshotId, bool includeGenerated = false)
    {
        EnsureOpen();
        return _declMaintenance!.ResolveSymbolByLocation(relativePath, line, snapshotId, includeGenerated);
    }
    ...
    {
        EnsureOpen();
        _edgeOps!.SaveEdges(snapshotId, edges);
    }
    ...
    {
        EnsureOpen();
        return _edgeOps!.GetEdges(snapshotId, symbolId);
    }
    ...
    {
        EnsureOpen();
        _annotations!.DeleteAnnotationsByDocumentPaths(snapshotId, documentPaths);
    }

    public OrphanEdgeDropSummary DeleteOrphanEdges(string snapshotId)
    {
        EnsureOpen();
        return _edgeOps!.DeleteOrphanEdges(snapshotId);
    }

    public void PruneSnapshotGraphNodes(string snapshotId)
    {
        EnsureOpen();
        _edgeOps!.PruneSnapshotGraphNodes(snapshotId);
    }
    ...
    {
        EnsureOpen();
        return _extractors!.HasStaleExtractorVersions(snapshotId);
    }
    ...
    {
        EnsureOpen();
        _searchMaintenance!.BuildSearchIndex(snapshotId);
    }
    ...
    {
        EnsureOpen();
        _searchMaintenance!.BuildSearchIndex(snapshotId, changedDocumentPaths, changedSymbolIds);
    }
    ...
    {
        EnsureOpen();
        _searchMaintenance!.CopySearchIndexToSnapshot(fromSnapshotId, toSnapshotId);
    }
    ...
    {
        EnsureOpen();
        return _searchSource!.SearchSource(query, snapshotId, limit, includeGenerated, snippetTokens);
    }
    ...
    {
        EnsureOpen();
        return _searchSymbols!.SearchSymbols(query, snapshotId, limit, includeGenerated, kind);
    }
    ...
    {
        EnsureOpen();
        return _searchSymbols!.SearchSymbolsPage(query, snapshotId, limit, includeGenerated, kind, cursor);
    }

    public IndexedSymbolInfo? ResolveSymbolByFqn(string fqn, string snapshotId, bool includeGenerated = false)
    {
        EnsureOpen();
        return _searchSymbols!.ResolveSymbolByFqn(fqn, snapshotId, includeGenerated);
    }
    ...
    {
        EnsureOpen();
        return _declReader!.LoadTransitionCandidates(snapshotId, symbolIds);
    }
    ...
    {
        EnsureOpen();
        return _semanticDiffStore!.GetSemanticChanges(fromSnapshotId, toSnapshotId);
    }

    public List<SemanticChange> GetSemanticChangesToSnapshot(string toSnapshotId)
    {
        EnsureOpen();
        return _semanticDiffStore!.GetSemanticChangesToSnapshot(toSnapshotId);
    }
