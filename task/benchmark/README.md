...
This benchmark implements T11 from `task/task_list.txt`. It measures agent-facing outcomes for three architecture scenarios, using a small committed multi-project fixture rather than database size or edge counts:
...
The fixture is in `fixtures/OutcomeBenchmark/`. It has no external package dependencies. The test project defines a minimal `FactAttribute`, which is sufficient for Lurp's test adapter while keeping the benchmark reproducible without a test framework restore.

## Recorded format

`tests/OutcomeBenchmarkTests.cs` is the repeatable runner. It writes `runs/baseline.json` (or the path in `LURP_OUTCOME_BENCHMARK_OUTPUT`) with one record per scenario:
...
```powershell
$env:LURP_OUTCOME_BENCHMARK_OUTPUT = "task/benchmark/runs/baseline.json"
dotnet test tests/Lurp.Storage.Tests.csproj --filter "FullyQualifiedName~OutcomeBenchmarkTests" --no-restore --no-build --verbosity normal -m:1
```
